...
import com.monitor.dal.User.dao.UserDAO;
...
public class UserImpl implements UserService {
    ...
    public String userLogin(UserDO userDO) {
        ...
        //密码加密
        userDO.setUSER_PWD(encryptionService.Md5Creator(userDO.getUSER_PWD()));
        //密码比对
    ...
    private String paramCheck(UserDO userDO){
        ...
        //检查用户名是否包含特殊字符
        if(userCheckService.IsCheckSpecialCharactors(userDO.getUSER_ACCT())){
            return ErrorCodeEnum.USER_ACCT_CANNOT_HAVE_SPECIAL_CHARACTORS.getIndex().toString();
        }
        //检查用户名是否包含中文字符
        if (userCheckService.IsCheckChineseCharactors(userDO.getUSER_ACCT())){
